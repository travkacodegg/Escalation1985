...
/obj/machinery/telecomms
	...
	var/listening_levels  = null	// 0 = auto set in New() - this is the z level that the machine is listening to.
...
/obj/machinery/telecomms/proc/checkheat()
	...
	switch(environment.temperature)
		if((T0C + 21) to (T0C + 70))                // 40C-70C, minor overheat, 10% chance of taking damage
			damage_chance = 10
...
/obj/machinery/telecomms/proc/produce_heat()
	...
	if(!(stat & (NOPOWER|BROKEN)))
		...
		if(istype(L))
			...
			var/transfer_moles = 0.4 * env.total_moles
			...
			if(removed)
				...
				if (traffic <= 0)
					heat_produced *= 0.60	//if idle, produce less heat.
