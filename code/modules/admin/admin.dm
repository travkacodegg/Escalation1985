...
/datum/admins/proc/toggleahelp()
	set category = "Server"
	set desc="Globally Toggles Adminhelp"
	set name="Toggle AHelp"

	if(!check_rights(R_ADMIN))
		return

	config.ahelp_allowed = !(config.ahelp_allowed)
	if (config.ahelp_allowed)
		to_world("<B>The adminhelp channel has been globally enabled!</B>")
	else
		to_world("<B>The adminhelp channel has been globally disabled!</B>")
	log_and_message_admins("toggled adminhelp.")
