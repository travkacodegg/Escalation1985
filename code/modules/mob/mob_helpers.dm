...
/mob/living/carbon/human/get_screen_colour() //Fetch the colour matrix from wherever (e.g. eyes) so it can be compared to client.color.
	...
	if(global_colour_matrix)
		...
		var/newcolor1 = (1 - (get_blood_oxygenation()/100)) * 0.33
...
/mob/proc/flash_eyes(var/time = 25, intensity = FLASH_PROTECTION_MODERATE, override_blindness_check = FALSE, affect_silicon = FALSE, visual = FALSE, type = /obj/screen/fullscreen/flash)
	return
